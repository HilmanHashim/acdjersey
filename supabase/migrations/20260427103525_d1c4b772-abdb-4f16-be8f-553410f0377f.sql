...
DROP POLICY IF EXISTS "Authenticated users can manage invoice sequences" ON public.invoice_sequences;
...
BEGIN
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION 'You must be signed in to update invoice sequences';
  END IF;
